...
class Grid(gym.Env):
    ...
    def init_agent(self, initial_pos=None):
        ...
        if initial_pos is not None:
            ...
            self.agent_orientations = [self.EAST, self.WEST]
            for i in range(self.n_agents):
                self.grid_status[self.agent_pos[i][0], self.agent_pos[i][1]] = 1
        else:
            self.agent_orientations = [self.EAST, self.WEST, self.WEST]
            for i in range(self.n_agents):
    ...
    def get_agent_obs(self):
        return {
            ...
            'global_state': {
                'drone_positions': np.array(self.agent_pos),
                'drone_orientations': np.array(self.agent_orientations),
                'grid_status': self.grid_status.copy()
            }
        }
    ...
    def get_new_position(self, x, y, orientation):
    ...
    def step(self, action, i): 
        ...
        if action == self.TURN_LEFT:
            ...
            reward -= 0.01
            new_orientation = (current_orientation + 2) % 4
        else:
            new_orientation = current_orientation

        self.agent_pos[i][0], self.agent_pos[i][1] = self.get_new_position(org_x, org_y, new_orientation)
    ...
    for episode in pbar:
        ...
        for step in range(max_steps):
            ...
            for agent_i in action_order:
                ...
                next_state = next_state['local_obs']
            ...
            efficiency = mapped_poi / max(steps, 1)

            if current_coverage >= coverage_threshold and coverage_track:
                metrics['speed'].append(steps)
                coverage_track = False

            if done:
                break
            elif step == max_steps - 1 and coverage_track:
                metrics['speed'].append(np.nan)
